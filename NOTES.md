...
Also updated the "Upcoming Availability" widget's heading, which still said "This Week's Availability" — stale copy from before the rolling-window fix.

## Post-Stage-4 fixes: past time slots

**Report:** booking a today slot whose time had already passed said the slot was "booked" (misleading — it was never actually reserved by anyone), and the visual calendar widget kept showing already-passed times for today as if they were still bookable.

**Fix 1 — calendar widget showing past times.** `GET /api/calendar` returned every slot for every day in the window with just a `booked` true/false flag, with no awareness of "already past." A slot that had simply gone by (never booked) rendered identically to a genuinely open one. Added `getPublicCalendar()` to `calendar.js`, which filters out already-passed slots before returning — this only ever affects today's date, since future days have no past times by definition. `GET /api/calendar` in `index.js` now calls this instead of hand-rolling the same filtering inline.

**Fix 2 — ambiguous/misleading wording.** `checkAvailability`'s day-specific "no open slots" message said "it's fully booked or already passed" — conflating two different reasons into one ambiguous phrase, which is likely why Claude sometimes described a passed time as "booked." Replaced with two distinct, situation-specific messages (whether the day's hours have fully ended vs. genuinely booked by others). `bookAppointment`'s past-time rejection now explicitly states *"it was not booked by anyone, that time has simply gone by"* and lists any later times still open that day, rather than a bare "has already passed."

**Fix 3 — Claude fabricating a reason instead of calling the tool for "today."** While testing (the local clock happened to cross midnight mid-session — good real-world validation that the rolling window now correctly starts Monday when "today" becomes a weekend), asking to book "today" produced *"same-day bookings aren't available through this system"* — a plausible-sounding but entirely made-up explanation; the `[tool]` debug log confirmed no tool call happened at all for that request. The prior prompt fix (Bug 3 above) told Claude to always call the tool for "any specific date the user mentions," but apparently didn't register relative terms like "today" as covered. **Fix:** strengthened the prompt to explicitly name relative date terms ("today", "tomorrow", a weekday name) as requiring the same tool-call treatment, and explicitly forbid inventing any explanation not sourced from the tool's actual response. Verified with a 3x repeat of the exact failing case — all three now correctly call the tool and relay its real reasoning; also reverified a normal future booking still works end-to-end after this change.

## Post-Stage-4 fix: booking "Monday" silently booked Tuesday

**Report:** user booked Monday 10am; the calendar showed Tuesday 10am crossed out instead.

**Reproduced exactly:** asked to "book Monday at 10am" — the reply said *"confirmed for Monday, July 21st"*, but July 21, 2026 is actually a Tuesday. The `[tool]` log confirmed the tool itself was called correctly and booked exactly the date it was given (`2026-07-21`) — `calendar.js` and the calendar widget were both working correctly on the data they received. The bug was entirely Claude computing the wrong absolute date for the weekday name "Monday" in its own head, then confidently mislabeling it in the reply too — the same class of date-arithmetic error as the earlier boundary-rejection bug (Bug 3, two sections up), just manifesting as a wrong-but-confident answer instead of a wrong rejection.
